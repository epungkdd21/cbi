...
export default function Page() {
  ...
  const [siteSettings, setSiteSettings] = useState({ hero_title: '{siteSettings.hero_title}', hero_description: 'Saksikan pertunjukan drama santri Pesantren Jagatarsy yang mengenalkan kekayaan budaya Indonesia melalui panggung Dwipantara.', ticket_price: String(ticketPrice), event_location: 'Jakarta' })

  useEffect(() => { fetch('/api/settings').then((response) => response.json()).then((data) => setSiteSettings((current) => ({ ...current, ...(data.settings || {}) }))).catch(() => undefined) }, [])
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-5 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Pentas budaya santri Nusantara</p>
            </div>
          </div>
        ...
        <div>
          <div className="mb-8 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-4 max-w-xl text-sm leading-6 text-muted-foreground">Saksikan pertunjukan drama santri Pesantren Jagatarsy yang mengenalkan kekayaan budaya Indonesia melalui panggung Dwipantara.</p>
            </div>
          ...
          {step === 0 && <section className="event-dashboard" aria-labelledby="event-heading"><div className="event-hero"><div><p className="eyebrow">Dwipantara / Event resmi</p><h2 id="event-heading" className="mt-3 max-w-2xl text-balance font-serif text-5xl leading-none sm:text-7xl">{siteSettings.hero_title}</h2><p className="mt-5 max-w-xl text-sm leading-6 text-muted-foreground">Hadiri performance drama santri Pesantren Jagatarsy: perjalanan budaya Indonesia yang dihidupkan melalui cerita, musik, tari, dan panggung Dwipantara.</p><button type="button" onClick={() => setStep(1)} className="primary-button mt-7">Beli tiket sekarang <span>→</span></button></div><div className="event-badge"><span>LIVE</span><strong>20—23</strong><small>JUNI 2025<br />JAKARTA</small></div></div><div className="event-details"><div><span>Acara</span><strong>Pemutaran film &amp; temu komunitas</strong></div><div><span>Lokasi</span><strong>{siteSettings.event_location}</strong></div><div><span>Durasi</span><strong>Performance budaya langsung</strong></div></div><div className="event-columns"><div><p className="eyebrow">Tentang acara</p><h3 className="mt-2 text-2xl font-semibold">Bukan sekadar pertunjukan.</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">Dwipantara adalah panggung bagi santri Pesantren Jagatarsy untuk mengenalkan budaya Indonesia melalui drama, musik, gerak, dan kisah yang diwariskan lintas generasi.</p></div><div className="event-list"><div><span>01</span><p><strong>Pentas drama</strong><br />Saksikan santri membawakan kisah budaya Indonesia di atas panggung.</p></div><div><span>02</span><p><strong>Warisan budaya</strong><br />Mengenal nilai, cerita, dan ragam budaya Nusantara bersama Dwipantara.</p></div></div></div></section>}
          ...
          {step === 1 && <section aria-labelledby="schedule-heading">
            ...
            <div className="mt-7 grid gap-6 md:grid-cols-2">
              <label className="field-label">Pilih lokasi pentas<select value={selectedCinema} onChange={(event) => setSelectedCinema(event.target.value)} className="select-field">{cinemas.map((cinema) => <option key={cinema}>{cinema}</option>)}</select></label>
